...
def read_espn_team_manager_map(
    mapping_path: Path,
) -> tuple[dict[int, dict[int, str]], dict[str, str], dict[int, dict[str, str]]]:
    ...
    season_team_names = {
        int(season): {
            manager_id: team_name
            for manager_id, team_name in team_map.items()
        }
        for season, team_map in raw_mapping.get("teamNames", {}).items()
    }
...
def team_name_for(
    season_team_names: dict[int, dict[str, str]],
    season_year: int,
    manager_id: str,
    fallback: str,
) -> str:
    return season_team_names.get(season_year, {}).get(manager_id, fallback)
...
def append_espn_data(
    ...
    season_team_manager_ids, display_names, season_team_names = read_espn_team_manager_map(mapping_path)
...
def build_historical_data(
    ...
    _, _, season_team_names = read_espn_team_manager_map(espn_mapping_path)
    ...
    season_manager_names: dict[int, set[str]] = defaultdict(set)
    week_numbers: dict[int, set[int]] = defaultdict(set)
    for record in records:
        season_manager_names[record.season].update([record.team1, record.team2])
        week_numbers[record.season].add(record.week)

    teams = []
    for year in years:
        for manager_name in owner_order:
            if manager_name not in season_manager_names[year]:
                continue
            teams.append(
                {
